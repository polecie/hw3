...
def save_menu(menus):
    ...
    for id, item in enumerate(menus, 1):
        df = df.append({'A': id, 'B': item['title'], 'C': item['description']}, ignore_index=True)
        for id_sub, sub_item in enumerate(item['submenus'], 1):
            df = df.append({'B': id_sub, 'C': sub_item['title'], 'D': sub_item['description']}, ignore_index=True)
            for id_dishes, descript in enumerate(sub_item['dishes'], 1):
                df = df.append(
                    {'C': id_dishes, 'D': descript['title'], 'E': descript['description'], 'F': descript['price']},
                    ignore_index=True)
